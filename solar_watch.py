...
broker needs a keepalive every 30 s to keep publishing, which this sends.
    ...
    python3 solar_watch.py --host <cerbo-ip>[,<second-ip>] --portal <vrm-portal-id> \
        --out trace.jsonl [--hours 12] [--vebus 276] [--battery 200]

--host defaults to $CERBO_HOST and --portal to $CERBO_PORTAL (the N/<portal>/...
topic prefix); neither is stored here, this repo is public.
...
def derive(latest, previous):
    ...
    failing that the trace keeps what it had -- the order in which the
    services being traced resolve it.
    """
...
def main():
    ...
    ap.add_argument('--host', default=os.environ.get('CERBO_HOST'),
                    help='comma-separated addresses to try in turn (wifi0 first, eth0 second); '
                         'default $CERBO_HOST')
    ap.add_argument('--portal', default=os.environ.get('CERBO_PORTAL'),
                    help='VRM portal id, the N/<portal>/... topic prefix; default $CERBO_PORTAL')
    ...
    if args.summary:
        ...
        return
    if not args.host or not args.portal:
        ap.error('give --host and --portal, or set CERBO_HOST and CERBO_PORTAL')
    record(args)
