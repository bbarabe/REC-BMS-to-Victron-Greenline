...
The pure engine emits shore, sustain floor/ceiling/two-sided hold/release and bounded boost intentions through REC protocol v2 (the two-sided hold re-anchors on arrival at its destination when its servo carries a real lift, dbus-recbms 3.3.2; since dbus-recbms 3.6.0 it regulates its destination by voltage: the solar band closes on arrival, every charger is commanded the hold voltage and the charge current limit is the BMS's own since dbus-recbms 3.7.0, the floor's `PV + charge_limit_a` brake being off by default; a near-zero figure that makes DVCC switch an MPPT off is never published). It never writes `IgnoreAcIn` directly. The shore AC input is resolved at runtime by both services with one shared rule (GX AC input types, else the settled input, else the Quattro's accepted or available input; dbus-recbms 3.4.0 / consumer 3.5.0), never hard-coded. With the Quattro's own "Prefer renewable energy" on (shore on AC in 2), REC manages that toggle once a day: solar from dawn, charge now from dusk in CHARGE and HOLD, solar all night in DISCHARGE, with a one-point daytime deficit fallback (dbus-recbms 3.5.0). REC checks generation, lease, actual feedback, command safety, connected dwell and relay rate limits. Accounting and historical trials do not select operating phases or veto departures. Fresh measured Quattro V×I replaces inferred net power; observed timestamps distinguish unchanged values from stale data.
